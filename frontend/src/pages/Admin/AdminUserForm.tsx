    ...
    <div className="p-16 border border-gray-200 rounded-2xl dark:border-gray-700 dark:bg-gray-900 shadow-sm max-w-5xl mx-auto w-full">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 w-full">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          {/* Max Cards (Super Admin Only) */}
          {currentUser?.role_id === 1 && (
            <div>
              <label className="block mb-1 text-sm font-medium dark:text-gray-300">Max Cards Allowed <span className="text-gray-500 text-xs ml-2">(1–1000)</span></label>
              <input type="number" min={1} max={1000} value={form.max_cards} onChange={(e) => setForm({ ...form, max_cards: e.target.value })} className={`w-full border px-3 py-2 rounded-lg text-lg dark:bg-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 ${errors.max_cards ? "border-red-500 focus:ring-red-500" : "focus:ring-blue-500"}`} />
              {errors.max_cards && <p className="text-red-500 text-sm mt-1">{errors.max_cards}</p>}
            </div>
          )}
        </div>
